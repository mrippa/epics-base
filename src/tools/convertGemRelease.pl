...
#
# Adopted for Gemini ADE. Added "dbPaths" argument. Philip Taylor, July 2014.
# Added "schPaths" argument. Mike Westfall 12 June 2017.
#
...
unless (@ARGV == 1) {
    ...
    print "\tdbPaths - send .database file paths from RELEASE macros to stdout\n";
    print "\tschPaths - send template schematics file paths from RELEASE macros to stdout\n";
...
for ($outfile) {
    ...
    /dbPaths/  	 	 and do { &dbPaths;		last; };
    /schPaths/   	 and do { &schPaths;		last; };
...
sub schPaths {
    ...
    foreach my $app (grep !/^TOP$/, @includes) {
      if (-d "$macros{$app}/templates/schematics") {
	print " $macros{$app}/templates/schematics";
      }
    }
}
